...
/* ─────────────────────────────────────────────
   AvatarMenu — YouTube 風アカウントメニュー
   デスクトップ: 右上 fixed
   モバイル: 左上 fixed（チャンネル FAB が右下にあるため）
   ───────────────────────────────────────────── */

export function AvatarMenu({
  user,
}: {
  user: { user_metadata: { avatar_url?: string; user_name?: string }; email?: string };
}) {
    ...
    >
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 w-64 rounded-2xl border-2 border-neon-pink bg-river-deep p-3 shadow-[4px_4px_0_var(--accent-cyan)] max-md:left-0 max-md:right-auto">
          {/* ユーザー名 */}
          <p className="mb-2 truncate border-b border-river-border pb-2 text-sm font-medium text-[var(--text-primary)]">
            {userName}
          </p>
          ...
          {/* ログアウト */}
          <form action={signOut} className="mt-2 border-t border-river-border pt-2">
            <button
              type="submit"
